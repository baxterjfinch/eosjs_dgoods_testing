...
class ResourceReportOption extends Option {
    ...
    process(optionValue, args) {
        if (optionValue != 'false') {
            ...
            const cliTable = new CLITable({ head: ['Contract', 'Action', 'Transaction ID',] });
                    ...
                    if (contract.hasOwnProperty(functionName)) {
                        ...
                        if (contractFunction.isTransactional) {
                            contractFunction.on('processed', (txReceipt) => {
                                console.log(txReceipt.processed.receipt.cpu_usage_us)
                                console.log(txReceipt.processed.receipt.net_usage_words)
                                console.log(txReceipt.processed.action_traces[0].account_ram_deltas[0].delta)
                                cliTable.push([contract.name, functionName, txReceipt.transaction_id]);
                            });
                        }
                    }
                }
