...
class ResourceReportOption extends Option {
    ...
    process(optionValue, args) {
        if (optionValue != 'false') {
            ...
            retrieveDeploymentsResources(args.eoslime, deploymentsResources);
            retrieveFunctionsResources(args.eoslime, functionsResources);
...
const retrieveDeploymentsResources = function (eoslime, deploymentsResources) {
    eoslime.Contract.on('deploy', (txReceipt, contractName) => {
        const deploymentResources = extractResourcesUsageFromReceipt(txReceipt);
        deploymentsResources.push([
            contractName,
            'deploy',
            deploymentResources.txId,
            deploymentResources.cpuCost,
            deploymentResources.netCost,
            deploymentResources.cpuCost
        ]);
    });
}

const retrieveFunctionsResources = function (eoslime, functionsResources) {
            ...
            if (contract.hasOwnProperty(functionName)) {
                ...
                if (contractFunction.isTransactional) {
                    contractFunction.on('processed', (txReceipt) => {
                        const functionResources = extractResourcesUsageFromReceipt(txReceipt);
...
const extractResourcesUsageFromReceipt = function (receipt) {
