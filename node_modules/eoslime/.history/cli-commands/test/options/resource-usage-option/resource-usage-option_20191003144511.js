...
const filterByMaxAndMinResources = function (contractsUsage) {
    ...
    for (let i = 0; i < contractsUsage.length; i++) {
        ...
        for (let j = 0; j < contractUsage.functionsResources.length; j++) {
            ...
            getMaxAndMinResourcesUsage(contractFunction);
            const minCPUResource = Math.min(...contractFunction);
            const maxCPUResource = Math.max(...functionResources);

            const minCPUResource = Math.min(...functionResources);
            const maxCPUResource = Math.max(...functionResources);
