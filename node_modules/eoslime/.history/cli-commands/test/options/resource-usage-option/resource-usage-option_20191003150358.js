            ...
            if (contract.hasOwnProperty(functionName)) {
                ...
                if (contractFunction.isTransactional) {
                    contractFunction.on('processed', (txReceipt) => {
                        ...
                        contractsResources[contract.name].functions[]
                        getMinMaxPair();

                        usageByContract.push([
                            contractUsage.contractName,
                            contractFunction.name,
                            contractFunctionResources.ram,
                            contractFunctionResources.cpu,
                            contractFunctionResources.net,
                            contractFunction.calls,
...
const filterByMaxAndMinResources = function (contractsUsage) {
    const usageByContract = [];
    for (let i = 0; i < contractsUsage.length; i++) {
        const contractUsage = contractsUsage[i];
        for (let j = 0; j < contractUsage.functionsResources.length; j++) {
            const contractFunction = contractUsage.functions[i];
            const contractFunctionResources = getMaxAndMinResourcesUsage(contractFunction);
...
const getMaxAndMinResourcesUsage = function (contractFunction) {
    let ram = [];
    let cpu = [];
    let net = [];

    for (let i = 0; i < contractFunction.calls.length; i++) {
        const ramUsage = contractFunction.ram[i];
        const cpuUsage = contractFunction.cpu[i];
        const netUsage = contractFunction.net[i];

        ram = getMinMaxPair(ram[0], ram[1], ramUsage);
        cpu = getMinMaxPair(cpu[0], cpu[1], cpuUsage);
        net = getMinMaxPair(net[0], net[1], netUsage);
    }
