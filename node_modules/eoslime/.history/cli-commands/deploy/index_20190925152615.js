...
class DeployCommand extends Command {
    ...
    async execute(args) {
        try {
            ...
            await runDeploymentScripts(optionsResults.path, { eoslime: optionsResults.network, deployer: optionsResults.deployer });
        } catch (error) {
...
const runDeploymentScripts = async function (deploymentScripts, options) {
    for (let i = 0; i < deploymentScripts.length; i++) {
        const deploymentScript = deploymentScripts[i];
        try {
            await deploymentScript.deploy(...options);
