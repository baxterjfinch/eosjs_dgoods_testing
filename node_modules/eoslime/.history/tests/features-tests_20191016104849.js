...
describe.only('Features', function () {
    ...
    beforeEach(async () => {
        try {
            ...
            mutisigContract = await eoslime.Contract.deployOnAccount(MSIG_WASM_PATH, MSIG_ABI_PATH, msig);
        } catch (error) {
            console.log(error);
        }
    });
    ...
    describe.only('Set authority abilities', function () {
        it('Should execute an action from custom authority', async () => {
            ...
            const msigAcc = eoslime.Contract.fromFile(MSIG_ABI_PATH, 'eosio.msig');
            const proposalData = await multiSigAccount.propose(msigAcc, faucetAccount.name, 'produce', faucetAccount.name, "100.0000 TKNS", faucetAccount.name, "memo")

            console.log(await msigAcc.getProposal({ equal: accounts[0].name }))
            // await multiSigAccount.approveProposal(accounts[1], proposalData)
            // await multiSigAccount.execProposal(proposalData, accounts[0]);
