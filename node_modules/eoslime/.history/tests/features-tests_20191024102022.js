...
describe.only('Features', function () {
    ...
    describe.only('Set authority abilities', function () {
        it.only('Should execute an action from custom authority', async () => {
            ...
            const multiSigAccount = eoslime.Account.loadMultisig(account.name, account.privateKey, 'active');
        ...
        it.only('Should execute an action from custom authority', async () => {
            ...
            const multiSigAccount = await eoslime.Account.loadMultisig(multiSigAuth.name, multiSigAuth.privateKey, 'multisig')
            const proposalId = await multiSigAccount.proposeKey(faucetContract.produce, [multiSigAccount.name, "100.0000 TKNS", multiSigAccount.name, "memo"])
    ...
    it('Should look beautiful', async () => {
        // const contract = eoslime.Contract.at('', '');
        // const multiSignature = await eoslime.Account.loadMultisig('proposer name', 'proposer private key', 'proposer authority');
        // multiSignature.privateKeys.push(keys)
        // multiSignature.accounts.push(account)

        // const proposal = multiSignature.propose(contract.produce, [actionData]);
        // multiSignature.approve(approver, proposal);
        // multiSignature.allApprove(proposal);

        // const proposalId = contract.produce(...actionData, { from: multiSignature })
        // multiSignature.approve(multiSignature.keys[0], proposalId)
    });
});
