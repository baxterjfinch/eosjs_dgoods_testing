    ...
    */
    ...
    describe('Instantiation', function () {
        ...
        describe.only('Set authority abilities', function () {
            ...
            beforeEach(async () => {
                faucetContract = await eoslimeTool.Contract.deploy(FAUCET_WASM_PATH, FAUCET_ABI_PATH);
            });

            it.only('Should set authority abilities and execute a granted action', async () => {
                const account = await eoslimeTool.Account.createRandom();

                const accountRandomAuth = await account.createAuthority('random');

                try {
                    await faucetContract.producer(account.name, "100.0000 TKNS", account.name, "memo", { from: accountRandomAuth });
                } catch (error) {
                    console.log(error);
                }
    ...
    describe('Create from name', function () {
        ...
        it('Should create account from name [account creator]', async () => {
            ...
            let accountName = eoslime.utils.randomName();
