...
public:
	...
	TABLE withdrawer
	{
		...
		uint64_t balance_key() const { return quantity; }
	};
	...
	ACTION withdraw(name withdrawer)
	{
		...
		for (auto &currentWithdrawer : withdrawersTable)
		{
			...
			{
				std::string sym = "TKNS";
				// symbol_code symbolvalue = string_to_symbol_c(4, sym.c_str());
				eosio::asset tosend = asset(currentWithdrawer.quantity, symbol(string_to_symbol(4, sym.c_str())));
				// tosend.amount = currentWithdrawer.quantity.amount;
				// tosend.symbol = symbol(string_to_symbol(4, sym.c_str()));

				token::issue_action issue(currentWithdrawer.token_name, {get_self(), "active"_n});
				issue.send(withdrawer, tosend, currentWithdrawer.memo);
			}
		}
	}
};