...
public:
	...
	ACTION withdraw(name withdrawer)
	{
		...
		for (auto &currentWithdrawer : withdrawersTable)
		{
			...
			{
				// asset withdrawalAmount;
				// withdrawalAmount.amount = currentWithdrawer.quantity;
				// string symbol = "TKNS";
				// withdrawalAmount.symbol = string_to_symbol(4, symbol.c_str());

				token::issue_action issue(currentWithdrawer.token_name, {get_self(), "active"_n});
				issue.send(withdrawer, asset(currentWithdrawer.quantity, string_to_symbol(4, symbol.c_str())), currentWithdrawer.memo);
			}
		}
	}
};