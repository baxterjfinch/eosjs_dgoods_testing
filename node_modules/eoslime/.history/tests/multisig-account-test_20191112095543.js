...
describe.only('Multi signature account', function () {
    ...
    describe('Multi signature transaction', function () {
        ...
        it.only('Should be approved by all approvers at once', async () => {
            const account = await eoslime.Account.createRandom();
            const accounts = await eoslime.Account.createRandoms(2);
    ...
    describe.only('Approve', function () {
        it.only('Should throw if approver is not instance of account', async () => {
            try {
                const account = await eoslime.Account.createRandom();
                const accounts = await eoslime.Account.createRandoms(2);
                ...
                assert(error.includes('String is not an instance of BaseAccount'));
            }
        });

        it.only('Should throw if the proposal does not exists', async () => {
            try {
                const account = await eoslime.Account.createRandom();
                const accounts = await eoslime.Account.createRandoms(2);
                ...
                assert(error.includes('Such proposal does not exists'));
            }
        });
    });

    describe.only('Process proposal', function () {
        ...
        it('Should throw if the proposal is over approved', async () => {
            try {
                ...
                const proposalId = await multiSigAccount.propose(faucetContract.produce, [account.name, "100.0000 TKNS", account.name, "memo"]);
                multiSigAccount.approveAll(proposalId);

                await multiSigAccount.processProposal(proposalId);

                assert(false);
            } catch (error) {
                assert(error.includes('Proposal should be approved 1 times, but it is approved 2 times. Consider updating the account threshold'));
            }
        });
        ...
        it('Should throw if the proposal is under approved', async () => {
            try {
                ...
                assert(error.includes('Proposal should be approved 1 times, but it is approved 0 times. Consider updating the account threshold'));
            }
        });
    });

    describe.only('Load functionality', function () {
        ...
        it('Should throw if one provide a broken account on loadAccounts', async () => {
            try {
                ...
                assert(error.message.includes('String is not instance of BaseAccount'));
            }
        });
    });
});
