...
describe('Multi signature account', function () {
    ...
    describe.only('Buy ram', function () {
        ...
        /*
            Deploy eosio.system contract on local nodoes in order to send eos and buy ram / bandwidth
        */
        // async function deployEOSIOSystem() {
        //     try {
        //         const eosioSystemAccount = await eoslime.Account.load('eosio', '5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3');
        //         eosioSystemContract = await eoslime.Contract.deployOnAccount(EOSIO_SYSTEM_WASM_PATH, EOSIO_SYSTEM_ABI_PATH, eosioSystemAccount);
        //     } catch (error) {
        //     }
        // }
        ...
        it.only('Should buy ram by self', async () => {
            ...
            let eosAccount = eoslime.Account.load(ACCOUNT_NAME, ACCOUNT_PRIVATE_KEY);

            // Send 10 EOS to the account in order to have enough balance to pay for his ram
            // await eosAccount.send(account, '10.0000', 'SYS');
            // console.log(await account.getBalance('SYS'))
            const buyRamProposal = await multiSigAccount.buyRam(10);
