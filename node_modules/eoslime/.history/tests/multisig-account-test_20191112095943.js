...
describe.only('Multi signature account', function () {
    ...
    describe.only('Process proposal', function () {
        ...
        it.only('Should throw if the proposal is over approved', async () => {
            try {
                const account = await eoslime.Account.createRandom();
                const accounts = await eoslime.Account.createRandoms(2);
        ...
        it.only('Should throw if the proposal is under approved', async () => {
            try {
                const account = await eoslime.Account.createRandom();
                const accounts = await eoslime.Account.createRandoms(2);
    ...
    describe.only('Load functionality', function () {
        it.only('Should throw if one provide a broken private key on loadKeys', async () => {
            try {
                const account = await eoslime.Account.createRandom();
                const multiSigAccount = eoslime.MultiSigAccount.load(account.name, account.privateKey);
                ...
                console.log(error)
                assert(error.message.includes('Non-base58'))
            }
        });
