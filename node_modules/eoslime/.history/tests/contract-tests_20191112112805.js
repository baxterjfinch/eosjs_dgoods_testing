...
describe("Contract", function () {
    ...
    */
    ...
    describe("Blockchain methods", function () {
        ...
        it.only('Should get the raw transaction from the action', async () => {
            const faucetContract = eoslime.Contract.fromFile(FAUCET_ABI_PATH, faucetAccount.name, faucetAccount);
            const tokensHolder = await eoslime.Account.createRandom();

            const rawActionTx = await faucetContract.produce.getRawTransaction(tokensHolder.name, '100.0000 TKNS', tokenContract.name, 'memo');
            assert(rawActionTx.expiration);
            assert(rawActionTx.ref_block_num);
            assert(rawActionTx.ref_block_prefix);
            assert(rawActionTx.max_net_usage_words);
            assert(rawActionTx.max_cpu_usage_ms);
            assert(rawActionTx.delay_sec);
            assert(rawActionTx.context_free_actions);
            assert(rawActionTx.actions);
            assert(rawActionTx.actions[0].name == 'produce');
            assert(rawActionTx.actions[0].account == faucetContract.name);
            assert(rawActionTx.actions[0].data);
            assert(rawActionTx.actions[0].authorization);
        });

        it.only('Should sign the action without broadcasting it', async () => {
            const faucetContract = eoslime.Contract.fromFile(FAUCET_ABI_PATH, faucetAccount.name, faucetAccount);
            const tokensHolder = await eoslime.Account.createRandom();
