...
describe.only('Features', function () {
    ...
    beforeEach(async () => {
        try {
            ...
            faucetAccount = await eoslime.Account.createFromName('eosio.msig')
            console.log(faucetAccount.privateKey)
            await eoslime.Contract.deployOnAccount(MSIG_WASM_PATH, MSIG_ABI_PATH, faucetAccount);
        } catch (error) {
            console.log(error);
        }
    });
