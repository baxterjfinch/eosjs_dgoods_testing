...
describe("Contract", function () {
    ...
    */
    ...
    describe("Blockchain methods", function () {
        ...
        it.only('Should pay tokens for the action', async () => {
            const faucetContract = eoslime.Contract.fromFile(FAUCET_ABI_PATH, faucetAccount.name, faucetAccount);
            const tokensHolder = await eoslime.Account.createRandom();

            const EOSIO = require('../src/defaults/local-account-default');
            console.log(await EOSIO.getBalance('SYS'));
            console.log(await faucetContract.executor.getBalance('SYS'));
            await faucetContract.produce(tokensHolder.name, '100.0000 TKNS', tokenContract.name, 'memo', { from: EOSIO, tokens: { value: 5 } });
            console.log(await faucetContract.executor.getBalance('SYS'));
            // assert(true);
        });
