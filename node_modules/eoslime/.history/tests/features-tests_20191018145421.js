...
describe.only('Features', function () {
    ...
    describe.only('Set authority abilities', function () {
        ...
        it.only('Should execute an action from custom authority', async () => {
            ...
            const firstKeyAcc = eoslime.Account.load(account.name, firstKey.privateKey)
            const secondKeyAcc = eoslime.Account.load(account.name, secondKey.privateKey)
            ...
            multiSigAuth.setAuthorityAbilities([
                {
                    action: 'propose',
                    contract: multiSigAccount.name
                },
                {
                    action: 'approve',
                    contract: multiSigAccount.name
            ...
            const multiSigAccount = await eoslime.Account.loadMultisig(firstKeyAcc.name, accounts, firstKey.privateKey, 'multisig')

            // mutisigContract = eoslime.Contract.fromFile(MSIG_ABI_PATH, 'eosio.msig', multisigAccount);


            const proposalData = await multiSigAccount.proposeKey(mutisigContract, faucetAccount.name, 'produce', account.name, "100.0000 TKNS", account.name, "memo")
