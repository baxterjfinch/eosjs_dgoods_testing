...
describe.only('Features', function () {
    ...
    beforeEach(async () => {
        try {
            ...
            faucetContract = await eoslime.Contract.deployOnAccount(FAUCET_WASM_PATH, FAUCET_ABI_PATH, faucetAccount, { inline: true });
            mutisigContract = await eoslime.Account.load('eosio.msig', '5KTopL7p1PvvjfmXnRihzFmyGQbEbDPk3W5eouu5AGZfoEYommU')
    ...
    describe.only('Set authority abilities', function () {
        it('Should execute an action from custom authority', async () => {
            ...
            await account.addOnBehalfAccount('eosio.msig', 'eosio.code', 2)
            ...
            mutisigContract = eoslime.Contract.fromFile(MSIG_ABI_PATH, 'eosio.msig');
            const proposalData = await multiSigAccount.propose(mutisigContract, faucetAccount.name, 'produce', faucetAccount.name, "100.0000 TKNS", faucetAccount.name, "memo")
