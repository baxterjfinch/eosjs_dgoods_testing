...
class ContractFunction extends EventClass {
    constructor(contract, functionName, contractStructs) {
        ...
        contractFunction.getRawTransaction = async function (...params) {
            const structuredParams = structureParamsToExpectedLook(params, contractStructs[functionName].fields);
            const functionTx = buildMainFunctionTx(contract.name, functionName, structuredParams, contract.executor);
            const rowTransaction = await contract.provider.eos.transaction(
                {
                    actions: functionTx.actions
                },
                { broadcast: false, sign: false }
            );

            return rowTransaction.transaction.transaction;
        }

        // Todo: should rework it 
        contractFunction.getSignedTransaction = async function (account, ...params) {
            const structuredParams = structureParamsToExpectedLook(params, contractStructs[functionName].fields);
            const functionTx = buildMainFunctionTx(contract.name, functionName, structuredParams, account);
            const rowTransaction = await contract.provider.eos.transaction(
                {
                    actions: functionTx.actions
                },
                { broadcast: false, sign: true, keyProvider: account.privateKey }
            );

            return rowTransaction.transaction;
        }

        contractFunction.getSignedTransaction_1 = async function (authAccount, account, ...params) {
            const structuredParams = structureParamsToExpectedLook(params, contractStructs[functionName].fields);
            const functionTx = buildMainFunctionTx(contract.name, functionName, structuredParams, authAccount);
            const rowTransaction = await contract.provider.eos.transaction(
                {
                    actions: functionTx.actions
                },
