...
import EOSAddon from '../addon-eos/eos-addon'
import EOSConnection from '../../types/eos-connection'
import QueryProcessor from './queries/query-chain-processor/query-processor'

class ReaderAddon extends EOSAddon {
    private allQueries: any = {};

    public constructor(eosConnection: EOSConnection) {
        super(eosConnection)
    }

    public select(table: string): any {
        const readerAddon = new ReaderAddon(this.eosConnection)
        const selectQuery = this.generateQueriesChain(readerAddon)
        return selectQuery.select(table)
    }

    private generateQueriesChain(readerAddon: ReaderAddon): Queries.SelectQuery {
        ...
        const queryProcessor = this.buildQueryProcessor(readerAddon.provider, readerAddon.allQueries);
    ...
    private buildQueryProcessor(provider: any, processableQueries: Array<any>) {
        return new QueryProcessor(provider, processableQueries);
    }
}

export default ReaderAddon
