...
class MultiSignatureAccount extends Account {
    ...
    async buyRam(bytes, payer = this) {
        ...
        const systemContract = await retrieveSystemContract.call(this, payer);
        validateSystemContractAction(systemContract, 'buyrambytes');
    ...
    async buyBandwidth(cpu, net, payer = this) {
        ...
        const systemContract = await retrieveSystemContract.call(this, payer);
        validateSystemContractAction(systemContract, 'delegatebw');
    ...
    async send(receiver, amount, symbol = 'EOS') {
        ...
        const systemContract = await retrieveContract.call(this, 'eosio.token', this);
        validateContractAction(systemContract, 'delegatebw');
        const contractFactory = new ContractFactory(this.provider);
        const systemContract = await contractFactory.at('eosio.token');
        return this.propose(systemContract.transfer, [
            this.name,
            receiver.name,
            `${amount} ${symbol}`,
...
const retrieveSystemContract = async function (executor) {
    const abiInterface = (await this.provider.eos.getAbi('eosio')).abi;
    return new Contract(this.provider, abiInterface, 'eosio', executor);
}

const validateSystemContractAction = async function (systemContract, action) {
    if (!systemContract[action]) {
        throw new Error('Missing system contract');
    }
}
