...
class MultiSignatureAccount extends Account {
    ...
    async approve(approver, proposalId) {
        ...
        if (approver.name == this.name) {
            const actionTx = await proposalAction.action.signAction(approver, ...proposalAction.data);
        } else {
            const actionTx = await proposalAction.action.signActionOnBehalf(approver, this, ...proposalAction.data);
        }
