...
class MultiSignatureAccount extends Account {
    ...
    async approve(approver, proposalId) {
        ...
        if (approver.name == this.name) {
            const actionTx = await proposalAction.action.signTransaction(approver, ...proposalAction.data);
        } else {
            const actionTx = await proposalAction.action.signTransactionOnBehalf(approver, this, ...proposalAction.data);
        }
