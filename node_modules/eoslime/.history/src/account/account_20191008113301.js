...
class Account {
    ...
    async addPermission(authorityName, weight = 1) {
        return this.addPermissionAccount(this.name, authorityName, weight);
    }

    async addPermissionAccount(accountName, authorityName, weight = 1) {
        const authorityInfo = await this.getAuthorityInfo();
        const hasAlreadyPermission = authorityInfo.required_auth.accounts.find((account) => {
    ...
    async addPermissionKey(publicKey, weight = 1) {
        const authorityInfo = await this.getAuthorityInfo();
        const hasAlreadyPermission = authorityInfo.required_auth.keys.find((key) => {
            console.log(key)
            // return account.permission.permission == authorityName;
        });

        if (!hasAlreadyPermission) {
            authorityInfo.required_auth.keys.push({ publicKey, weight });

            return setAuthority.call(this, authorityInfo.perm_name, authorityInfo.parent, authorityInfo.required_auth);
        }
    }

    // Todo: Test if the already existing threshold will replace the provided one
    async setThreshold(threshold) {
        const authorityInfo = await this.getAuthorityInfo();

        const authorization = {
            threshold,
            ...authorityInfo.required_auth
        }

        return setAuthority.call(this, authorityInfo.perm_name, authorityInfo.parent, authorization);
    }
