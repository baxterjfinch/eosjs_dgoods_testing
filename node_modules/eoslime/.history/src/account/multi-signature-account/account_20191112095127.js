...
class MultiSignatureAccount extends BaseAccount {
    ...
    async approve(approver, proposalId) {
        ...
        this.proposals[proposalId].signatures.push(approverSignedTx.transaction.signatures[0]);
    }
...
const requireEnoughApprovals = async function (account, proposal) {
    ...
    if (authorityInfo.required_auth.threshold != proposal.signatures.length) {
        throw new Error(`Proposal should be approved ${authorityInfo.required_auth.threshold - 1} times, but it is approved ${proposal.signatures.length - 1} times. Consider updating the account threshold`);
    }
}
