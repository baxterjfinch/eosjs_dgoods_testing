...
const requireEnoughApprovals = async function (account, proposal) {
    const accountAuthorityInfo = await account.getAuthorityInfo();
    console.log(accountAuthorityInfo)
    if (accountAuthorityInfo != proposal.signatures.length) {
        throw new Error(`Proposal should be approved ${accountAuthorityInfo} times, but it is approved ${proposal.signatures.length}`);
    }
}
