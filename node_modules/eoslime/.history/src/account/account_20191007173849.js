...
class Account {
    ...
    async setAuthorityAbilities(abilities) {
        ...
        await this.provider.eos.transaction((tr => {
            tr.linkauth({
                account: executor.name,
                code: contract.name,
                type: 'addadmin',
                requirement: 'custom'
            }, { authorization: [contract.executor.executiveAuthority] });

        }, { broadcast: true, sign: true, keyProvider: contract.executor.privateKey });
    }
    ...
    async setThreshold(threshold) {
        const accountInfo = await eoslime.Provider.eos.getAccount(this.name);
        const authority = accountInfo.permissions.find((permission) => {
            return account.executiveAuthority.permission == permission.perm_name;
        });

        await eoslime.Provider.eos.transaction(tr => {
            tr.updateauth({
                account: this.name,
                permission: authority.perm_name,
                parent: authority.parent,
