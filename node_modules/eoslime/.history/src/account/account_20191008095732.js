...
class Account {
    ...
    async getAuthorityInfo() {
        const accountInfo = await provider.eos.getAccount(this.name);
    ...
    async updateAuthority(authorization) {
        const authorityInfo = await getAuthorityInfo();

        return this.provider.eos.transaction(tr => {
            tr.updateauth({
                account: this.name,
                permission: authorityInfo.perm_name,
                parent: authorityInfo.parent,
                auth: authorization
            }, { authorization: [this.executiveAuthority] });
    ...
    async addAuthorityKey(publicKey, weight) {
        const authority = await this.getAuthorityInfo();
        authority.required_auth.keys.push({ publicKey, weight });

        return this.updateAuthority(authority.required_auth);
    }

    async addAuthorityActor(actor, weight) {
        const authority = await this.getAuthorityInfo();
        authority.required_auth.accounts.push({ permission: { actor: actor, permission: permName }, weight });
    ...
    async setThreshold(threshold) {
        ...
        return this.updateAuthority(authorization);
    }
...
// Private methods
const setAuthority = async function () {

}

// const getAuthorityFromBlockchain = async function (provider, account) {
//     const accountInfo = await provider.eos.getAccount(account.name);
//     const authority = accountInfo.permissions.find((permission) => {
