...
class Account extends BaseAccount {
        ...
        */
        await this.provider.eos.transaction(tr => {
            for (let i = 0; i < abilities.length; i++) {
                ...
                console.log(this.executiveAuthority.permission)
                tr.linkauth({
                    account: this.name,
                    code: ability.contract,
                    type: ability.action,
                    requirement: this.executiveAuthority.actor
    ...
    async addPermission(authorityName, weight = 1) {
        return this.addOnBehalfAccount(this.name, weight);
    }

    async addOnBehalfAccount(accountName, authority, weight = 1) {
        const authorityInfo = await this.getAuthorityInfo();
        ...
        if (!hasAlreadyAccount) {
            authorityInfo.required_auth.accounts.push({ permission: { actor: accountName, permission: this.executiveAuthority.permission }, weight });
