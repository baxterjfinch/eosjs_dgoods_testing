...
class MultiSignatureAccount extends BaseAccount {
    ...
    async proposeKey(contractName, action, ...actionData) {
        ...
        const contractFactory = new ContractFactory(this.provider)
        const contractInstance = await contractFactory.at(contractName, this);
        const actionTx = await contractInstance[action].getSignedTransaction(this.accounts[0], ...actionData);
    ...
    async approveKey(approver, contractName, actionName) {
        ...
        console.log(this.accounts[0].publicKey)
        console.log(this.accounts[0].executiveAuthority)
        console.log(this.accounts[1].publicKey)
        console.log(this.accounts[1].executiveAuthority)
        const actionTx = await this.contracts[contractName].instance[actionName].getSignedTransaction(this.accounts[1], ...this.contracts[contractName].params);
    ...
    async execKey(contractName, actionName) {
        if (!this.proposals[contractName]) {
            throw new Error('Such proposal does not exists')
        }
