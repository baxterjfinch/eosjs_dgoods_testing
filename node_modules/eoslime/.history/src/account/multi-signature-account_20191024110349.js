...
class MultiSignatureAccount extends Account {
    ...
    async propose(contractAction, actionData) {
        const actionTx = await contractAction.signTransaction(this, ...actionData);
    ...
    async approve(approver, proposalId) {
        ...
        if (approver.name == this.name) {
            const actionTx = await proposalAction.action.sign(approver, ...proposalAction.data);
        } else {
            const actionTx = await proposalAction.action.signOnBehalf(approver, this, ...proposalAction.data);
        }
    ...
    async processProposal(proposalId) {
        requireExistingProposal(this, proposalId);
...
const requireExistingProposal = function () {
    if (!this.proposals[proposalId]) {
        throw new Error('Such proposal does not exists');
    }
}
