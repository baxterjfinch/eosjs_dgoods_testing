...
class MultiSignatureAccount extends BaseAccount {
    ...
    loadAccount(privateKey) {
        new Account(this.name, privateKey, this.provider, this.executiveAuthority.permission);
    }
    ...
    async proposeKey(contractName, action, ...actionData) {
        ...
        if (!this.signatures[contractName]) {
            this.signatures[contractName] = {
                [action]: {}
            };
        }

        this.signatures[contractName][action].push(actionTx.signatures[0]);
        // const accountInfo = await this.getAuthorityInfo();
        // const requested = [];
        // for (let i = 0; i < accountInfo.required_auth.keys.length; i++) {
        //     requested.push({
        //         actor: accountInfo.required_auth.keys[i].key,
        //         permission: 'active'
        //     })
        //     // requested.push(accountInfo.required_auth.keys[i])
        // }
