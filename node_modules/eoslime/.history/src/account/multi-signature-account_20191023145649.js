...
class MultiSignatureAccount extends BaseAccount {
    ...
    async proposeKey(contractName, action, ...actionData) {
        const ContractFactory = require('./../contract/contract-factory');
        const contractFactory = new ContractFactory(this.provider)
        const contractInstance = await contractFactory.at(contractName, this);
        const actionTx = await contractInstance[action].getSignedTransaction(this, ...actionData);
        if (!this.proposals[contractName]) {
            this.proposals[contractName] = {
                [action]: {}
            };
        }

        this.contracts[contractName] = { instance: contractInstance, params: actionData };
        this.proposals[contractName][action] = actionTx;
    }
    ...
    async approveKey(approver, contractName, actionName) {
        is(approver).instanceOf(BaseAccount)
        if (!this.proposals[contractName]) {
            throw new Error('Such proposal does not exists')
        }

        const actionTx = await this.contracts[contractName].instance[actionName].getSignedTransaction(approver, ...this.contracts[contractName].params);
        this.proposals[contractName][actionName].signatures.push(actionTx.signatures[0]);
    }
    ...
    async execProposal(msig, contractName, executor = this) {
    ...
    async execKey(contractName, actionName, executor = this) {
        is(executor).instanceOf(BaseAccount);
        ...
        return pushMultiSignatureTransaction.call(this, proposalTx, executor)
        // return contract.instance[actionName](contract.params)
...
const pushMultiSignatureTransaction = async function (signedRawTx, executor) {
    return this.provider.eos.pushTransaction(signedRawTx, { broadcast: true, sign: true, keyProvider: executor.privateKey });
}
