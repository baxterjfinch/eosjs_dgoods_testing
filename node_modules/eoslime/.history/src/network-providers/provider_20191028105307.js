...
class Provider {
    constructor(network) {
        if (NETWORKS[network]) {
            return proxyfyProvider(NETWORKS[network]());
        }

        return proxyfyProvider(NETWORKS.custom(network));
    }
...
const proxyfyProvider = function (provider) {
    ...
    const providerProxy = new Proxy(class ProviderInstance {
        constructor(network) {
